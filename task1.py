...
from robo_algo.drawing_data import get_drawing1
...
# FK / Jacobian / IK: robo_algo.kinematics (Levenberg–Marquardt + optional nullspace).  #
#######################################################################################
#######################################################################################
            ...
            if controller.is_idle():
                if i_shape < len(drawing):
                    if i_point >= len(drawing[i_shape]):
                        i_shape += 1
                        i_point = 0
                    if i_shape < len(drawing) and i_point < len(drawing[i_shape]):
                        tgt = np.asarray(drawing[i_shape][i_point], dtype=float)
